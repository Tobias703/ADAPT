"""
transport.py — Transport base class and registry
...
Adding a new transport requires only:
  ...
  4. Decorate the class with @register  (or import it in transports/__init__.py)
...
class BaseTransport(ABC):
    """
    Abstract base for PT transports.

    A **new instance** is created for every connection, so subclasses may
    safely maintain per-connection state (session keys, sequence numbers, …).
    Stateless transports (like foobar) simply ignore the instance state.
    """

    #: Unique wire name used in torrc / PT IPC (e.g. "obfs4", "foobar").
    name: str = ""
    ...
    def encode(self, data: bytes) -> bytes:
        ...
        Contract:
          ...
          • ``encode(b'')`` must return ``b''``.
          • Calling code passes arbitrarily-sized chunks; encode each chunk
    ...
    def decode(self, buf: bytes) -> Tuple[bytes, bytes]:
        ...
        Contract:
          ...
          • Returns ``(decoded_output, remaining_buf)`` where ``remaining_buf``
            contains any bytes that could not yet form a complete frame.
          • ``decode(b'')`` must return ``(b'', b'')``.
          • The relay engine feeds ``remaining_buf`` back into the next call
            automatically.
        """
        ...
