...
async def _handle_connection(client_reader: asyncio.StreamReader, client_writer: asyncio.StreamWriter, transport_name: str, or_host: str, or_port: int) -> None:
    ...
    try:
        ...
        log.warning("[%s] Cannot connect to ORPort %s:%d - %s", peer, or_host, or_port, exc)
        try:
            client_writer.close()
        except Exception:
            pass
    ...
    await relay_mod.relay(client_reader, client_writer, or_reader, or_writer, a_to_b_fn=transport.decode, a_to_b_is_decode=True, b_to_a_fn=transport.encode, b_to_a_is_decode=False)
...
async def run_server(cfg: PTConfig) -> None:
    ...
    if cfg.ext_or_port:
        log.info("ExtORPort configured (%s) - using plain ORPort instead (ExtORPort framing not implemented)", cfg.ext_or_port)
    ...
    for name in cfg.transports:
        ...
        try:
            server = await asyncio.start_server(lambda r, w, n=name, h=or_host, p=or_port: _handle_connection(r, w, n, h, p), host=bind_host, port=bind_port)
        except OSError as exc:
        ...
        log.info("Transport %r server listener ready at %s  (ORPort -> %s:%d)", name, listen_addr, or_host, or_port)
