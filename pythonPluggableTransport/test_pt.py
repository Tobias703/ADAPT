...
class TestTransportUnit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not TARGET_TRANSPORT:
            self.skipTest("Target transport not specified. Please run the script with -t <transport>.")
...
class TestEndToEnd(unittest.TestCase):
    ...
    def setUpClass(cls):
        if not TARGET_TRANSPORT:
            raise unittest.SkipTest("Target transport not specified. Run with -t <transport>.")
        ...
        cmethod_line = cls.pt_client.wait_for_line(f'CMETHOD {TARGET_TRANSPORT} socks5 ')
