"""
config.py — PT configuration parser

Supports both:
  • PT 3.0 command-line flags  (-ptversion, -state, -transports, …)
  • PT 1.0/2.0 environment variables  (TOR_PT_MANAGED_TRANSPORT_VER, …)

Per spec, check the -ptversion flag first; if absent fall back to the
TOR_PT_MANAGED_TRANSPORT_VER environment variable.  This makes the
dispatcher backward-compatible with Tor, which still uses PT 1.0 env vars.
"""
...
IMPLEMENTATION_NAME = "pt-python-dispatcher"
...
class PTConfig:
    ...
    protocol_version: str                  # negotiated PT spec version string
    state_dir: str                         # writable state directory
    transports: List[str]                  # requested transport names

    # Client-only
    upstream_proxy: Optional[str] = None   # e.g. "socks5://127.0.0.1:9050"

    # Server-only
    or_port: Optional[str] = None          # "host:port" of ORPort
    ext_or_port: Optional[str] = None      # "host:port" of ExtORPort (optional)
    bind_addrs: Dict[str, str] = field(default_factory=dict)   # name → "host:port"
    server_options: Dict[str, str] = field(default_factory=dict)  # name → "k=v;…"
...
def _negotiate_version(versions_str: str) -> Optional[str]:
    """
    Given the parent's comma-separated supported-versions string,
    return the best mutually supported version or None.
    """
...
def _parse_server_transport_options(raw: str) -> Dict[str, str]:
    ...
    Format: "name:k=v;name2:k2=v2;…"
    Colons, semicolons, and backslashes are backslash-escaped.
    Returns {name: "k=v string"}.
    """
    result: Dict[str, str] = {}
    # Simple tokeniser that respects backslash escapes
    i = 0
    current = []
    segments = []
    while i < len(raw):
...
def parse_config() -> PTConfig:
    ...
    parser.add_argument('-mode',         default=None,
                        choices=['client', 'server', 'transparent-TCP',
                                 'STUN-aware-UDP', 'transparent-UDP'])
    args, _ = parser.parse_known_args()

    # ── 2. Version negotiation ────────────────────────────────────────────────
    # PT 3.0: -ptversion flag; PT 1.0/2.0: TOR_PT_MANAGED_TRANSPORT_VER env var
    versions_raw = (args.ptversion
                    or os.environ.get('TOR_PT_MANAGED_TRANSPORT_VER'))
    ...
    state_dir = (args.state
                 or os.environ.get('TOR_PT_STATE_LOCATION', '/tmp/pt_state'))
    ...
    if not mode:
        if os.environ.get('TOR_PT_CLIENT_TRANSPORTS'):
            ...
            sys.exit(1)
    # Map PT 3.0 transparent modes to the simpler label for internal use
    ...
    if mode == 'server':
        ...
        bind_raw = (args.bindaddr
                    or os.environ.get('TOR_PT_SERVER_BINDADDR', ''))
        if bind_raw:
